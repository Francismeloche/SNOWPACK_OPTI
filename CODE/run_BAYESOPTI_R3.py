...
def run_snowpack(parameters):
    ...
    for year in years:
        ...
        try:
            ...
            rscript_path = os.path.abspath("./CODE/SimilarityBAYES3.R")
            ...
            print('FINAL score :', (F1*0.4 + simi*0.4 + rmse*0.2))
            final = float(F1*0.4 + simi*0.4 + rmse*0.2)
            final_year.append(final)
    ...
    print('FINAL score year:', final_year)
    print('FINAL score year:', np.mean(final_year))
    return np.mean(final_year)


# Number of parallel evaluations per iteration
n_parallel = 24
n_iterations = 8 # Total Bayesian optimization iterations
